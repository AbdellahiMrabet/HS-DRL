...
class K8sEnvBaseline(gym.Env):
    ...
    def reset(self, seed=None):
        ...
        self.latest_response_time_normalized = {n['name']: self._normalize_response_time(100.0) for n in self.nodes}
    ...
    def select_best_node_eprs(self) -> int:
        """
        Select best node by simulating the EPRS reward for each possible action.
        
        Uses the EPRS reward function (Equation 13):
        r_t = w₁ * ū(t) - w₂ * I(t) - penalty1 - penalty2
        
        Returns:
            Best node index (or action_space.n - 1 for delay if no node is suitable)
        """
        
        pod_cpu = self.current_pod['cpu'] if self.current_pod is not None else 0
        pod_mem_mib = self.current_pod['mem'] if self.current_pod is not None else 0
        
        best_action = self.action_space.n  # Default to delay
        best_reward = -float('inf')
        
        # EPRS hyperparameters (same as step function)
        w1 = 10.0
        w2 = 10.0
        c1 = 0.5
        c2 = 0.25
        M = 2      # Resource dimensions (CPU, MEM)
        M_total = 3  # Including Pod density
        
        for action in range(len(self.nodes)):
            ...
            # --- Simulate projected state after deployment ---
            # Create a copy of current nodes
            projected_nodes = copy.deepcopy(self.nodes)
            target_node = projected_nodes[action]
            
            # Project new resource usage
            ...
            target_node['cpu_percent'] = min(projected_cpu_util, 1.0)
            target_node['mem_percent'] = min(projected_mem_util, 1.0)
            target_node['pods'] = target_node.get('pods', 0) + 1
            
            # --- Calculate ū(t): Average resource utilization ---
            node_avg_utils = []
            for n in projected_nodes:
                if n.get('ready', True):
                    cpu_util = n['cpu_percent']
                    mem_util = n['mem_percent']
                    disk_util = n.get('disk_percent', 0.0)
                    node_avg = (cpu_util + mem_util) / M
                    node_avg_utils.append(node_avg)
            
            avg_util = np.mean(node_avg_utils) if node_avg_utils else 0.0
            
            # --- Calculate I(t): Load imbalance ---
            ready_nodes = [n for n in projected_nodes if n.get('ready', True)]
            
            if ready_nodes:
                cpu_utils = np.array([n['cpu_percent'] for n in ready_nodes])
                mem_utils = np.array([n['mem_percent'] for n in ready_nodes])
                disk_utils = np.array([n.get('disk_percent', 0.0) for n in ready_nodes])
                
                # Pod density
                pod_counts = np.array([n.get('pods', 0) for n in ready_nodes])
                total_pods = pod_counts.sum()
                if total_pods > 0:
                    pod_density = pod_counts / total_pods
                else:
                    pod_density = np.ones(len(ready_nodes)) / len(ready_nodes)
                
                all_dims = np.vstack([cpu_utils, mem_utils, pod_density])
                dim_stds = np.std(all_dims, axis=1, ddof=1)
                imbalance = np.mean(dim_stds)
            else:
                imbalance = 0.0
            
            # --- Calculate penalty1: upper threshold violation (> 90%) ---
            penalty1 = 0.0
            for n in ready_nodes:
                if (n['cpu_percent'] > 0.9 or 
                    n['mem_percent'] > 0.9):
                    penalty1 = c1
                    break
            
            # --- Calculate penalty2: lower threshold violation (< 10%) ---
            penalty2 = 0.0
            for n in ready_nodes:
                if (n['cpu'] < 0.1 or 
                    n['mem'] < 0.1):
            ...
            if (projected_cpu_util > CPU_LIMIT or projected_mem_util > MEM_LIMIT):
                # Add extra penalty for constraint violation
                constraint_penalty = 2.0
            else:
                constraint_penalty = 0.0
            
            # --- Final EPRS reward (Equation 13) ---
            reward = w1 * avg_util - w2 * imbalance - penalty1 - penalty2 - constraint_penalty
            
            print(f"  🔍 Node {node_name}: Ū={avg_util:.4f}, I={imbalance:.4f}, "
                f"P1={penalty1}, P2={penalty2}, Constraint={constraint_penalty}, "
                f"Projected CPU={projected_cpu_util*100:.1f}%, "
                f"Projected MEM={projected_mem_util*100:.1f}%, "
                f"Reward={reward:.4f}")
        ...
        print(f"  ✅ Best action: Node {self.nodes[best_action]['name']} "
    ...
    def step(self, action):
        ...
        if action < len(self.nodes):
            ...
            if not target_node.get('ready', True):
                ...
                if deployed:
                    ...
                    # ============================================================
                    # EPRS REWARD FUNCTION
                    # r_t = w₁ * ū(t) - w₂ * I(t) - penalty1 - penalty2
                    # ============================================================
                    
                    # Hyperparameters from EPRS paper (Section 4.2)
                    w1 = 10.0      # Weight for average resource utilization
                    w2 = 10.0      # Weight for load imbalance
                    c1 = 0.5       # Penalty for exceeding upper threshold (90%)
                    c2 = 0.25      # Penalty for exceeding lower threshold (10%)
                    
                    # --- Calculate ū(t): Average resource utilization across all nodes ---
                    # Equation (2) & (3) from paper
                    # ū_n(t) = (1/(M-1)) * Σ_{m=1}^{M-1} ru_n^m(t)   -- per-node average util
                    # ū(t)   = (1/N) * Σ_{n=1}^{N} ū_n(t)             -- cluster average util
                    
                    M = 2  # Resource dimensions: CPU, memory (M-1 = 2)
                    # Note: In our env we track CPU and MEM only, using available nodes
                    
                    node_avg_utils = []
                    for node in self.nodes:
                        if node.get('ready', True):
                            # ru_n^m(t): resource utilization per dimension
                            node_cpu_util = node['cpu_percent']
                            node_mem_util = node['mem_percent']
                            # For disk I/O, we don't have real data, use 0.0 or estimate
                            # In a real EPRS implementation this would come from monitoring
                            node_disk_util = 0.0  # Placeholder for disk I/O utilization
                            
                            # Per-node average utilization (Equation 2)
                            node_avg = (node_cpu_util + node_mem_util) / M
                            node_avg_utils.append(node_avg)
                    
                    # Cluster average utilization (Equation 3)
                    if node_avg_utils:
                        avg_util = np.mean(node_avg_utils)
                    else:
                        avg_util = 0.0
                    
                    # --- Calculate I(t): Load imbalance metric ---
                    # Equation (5): I(t) = (1/M_total) * Σ_{m=1}^{M_total} std(ru_n^m across nodes)
                    # M_total = 3 dimensions: CPU, memory, and Pod density
                    
                    M_total = 3  # M dimensions including Pod density
                    ready_nodes = [n for n in self.nodes if n.get('ready', True)]
                    
                    if ready_nodes:
                        # Extract per-dimension utilization vectors across nodes
                        cpu_utils = np.array([n['cpu_percent'] for n in ready_nodes])
                        mem_utils = np.array([n['mem_percent'] for n in ready_nodes])
                        disk_utils = np.zeros(len(ready_nodes))  # Placeholder
                        
                        # Pod density: ru_n^M(t) = p_n(t) / P(t)  (Equation 4)
                        # We estimate Pod count from node metrics or track separately
                        pod_counts = np.array([n.get('pods', 0) for n in ready_nodes])
                        total_pods = pod_counts.sum()
                        if total_pods > 0:
                            pod_density = pod_counts / total_pods
                        ...
                        all_dims = np.vstack([cpu_utils, mem_utils, disk_utils, pod_density])
                        
                        # I(t) = (1/M) * Σ_{m=1}^{M} std(ru_n^m across nodes)  (Equation 5)
                        dim_stds = np.std(all_dims, axis=1, ddof=1)  # Sample std (N-1 denominator)
                        imbalance = np.mean(dim_stds)
                    else:
                        imbalance = 0.0
                    
                    # --- Calculate penalty1: upper threshold violation (> 90%) ---
                    # Equation (14): penalty if any node resource > 0.9
                    penalty1 = 0.0
                    for node in ready_nodes:
                        if (node['cpu_percent'] > 0.9 or 
                            node['mem_percent'] > 0.9):
                            penalty1 = c1
                            break
                    
                    # --- Calculate penalty2: lower threshold violation (< 10%) ---
                    # Equation (15): penalty if any node resource < 0.1
                    penalty2 = 0.0
                    for node in ready_nodes:
                        if (node['cpu'] < 0.1 or 
                            node['mem'] < 0.1):
                    ...
                    # --- Final EPRS reward (Equation 13) ---
                    reward = w1 * avg_util - w2 * imbalance - penalty1 - penalty2
                    
                    print(f"  ✅ Deployed to {node_name} | API: {api_response_time:.1f}ms | "
                        f"Ū(t): {avg_util:.4f} | I(t): {imbalance:.4f} | "
                        f"P1: {penalty1} | P2: {penalty2} | "
                        f"Constraint: {projection_triggered} | Reward: {reward:.2f}")
                else:
        ...
        if self.tracker:
            ...
            imbalance_tracker = np.std([n['cpu_percent'] for n in self.nodes]) if self.nodes else 0.0
            
            self.tracker.step(
                reward=reward,
                success=success,
                util=avg_util_tracker,
                disk_io=0.0,
